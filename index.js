...
client.on(Events.GuildMemberRemove, async (member, err) => {
  ...
  if (!leavedata) return;
    ...
    const embedleave = new EmbedBuilder()
      ...
      .setThumbnail(
        "https://media.discordapp.net/attachments/1088740397406244896/1102607564115542056/IMG_20230430_182925.png"
      );
...
client.on(Events.GuildMemberAdd, async (member, err) => {
  ...
  if (!welcomedata) return;
    ...
    .setImage("https://images-ext-1.discordapp.net/external/hcjgh8vQt9mPoMyK5itLaK8dbgygrf4TOoBLoSfFXA8/https/images-ext-2.discordapp.net/external/axrZr_O_SY8SS-c6sXxma1FoUtDJ9pEaD2-qRRYc88A/%253Fq%253Dtbn%253AANd9GcQx-ejs3J176FZIoI8Czl-fGfqLXCAdAuggUw%2526usqp%253DCAU/https/encrypted-tbn0.gstatic.com/images")
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `> ${message.author}, links are **disabled** in **${message.guild.name}**.`
      )
      .setTimestamp();
        ...
        async (err, data) => {
          ...
          if (!data) {
            data = new warningSchema({
              ...
              Content: [
                {
                  ExecuterId: "1102156950889836564",
                  ExecuterTag: "Nexus Bot#8744",
  ...
  if (data.XP + give >= requiredXP) {
    ...
    const levelembed = new EmbedBuilder()
      ...
      .addFields({
        ...
        value: `> ${author.username} is now level **${data.Level}**!`,
      })
      .setAuthor({ name: `⬆ Level Playground` });

    await message.channel
      .send({ embeds: [levelembed] })
      .catch((err) => console.log("Error sending level up message!"));
  } else {
...
client.on(Events.MessageDelete, async (message) => {
  ...
  if (message.mentions.users.first() || message.type === MessageType.reply) {
    ...
    const ghostdmembed = new EmbedBuilder()
      ...
      .setTitle("Ghost pings are not Allowed")
      .setDescription(
        `> You were warned and timedout in **${message.guild.name}** for ghost pinging`
      );
  ...
  if (afkcheck) {
    ...
    if (message.content.includes(members)) {
      const m = await message.reply({
        content: `${member.user.tag} is currently AFK, let's keep it down.. \n> **Reason**: ${msg}`,
        ephemeral: true,
      });
      setTimeout(() => {
        m.delete();
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (
    message.content == "<@1102156950889836564>" ||
    (inputmessage === "hey Nexus" &&
      message.author.id !== "1102156950889836564")
  ) {
    const msg = await message.reply({
      content: `Hello there **${message.author}** :) Use </help manual:1081529934884917279> to get a list of my features!`,
      ephemeral: true,
    });
    setTimeout(() => {
      try {
  ...
  if (
    inputmessage.includes("Nexus sucks") &&
    message.author.id === "830703998750556201" &&
    message.author.id !== "1102156950889836564"
  ) {
    const msg = await message.reply({
      content: `Bro what the hell, I am literaly your son 😭`,
    });
    setTimeout(() => {
      try {
        msg.delete();
...
client.on(Events.InteractionCreate, async (interaction, err) => {
  const helprow2 = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .addOptions(
...

        {
          ...
          description: "Displays how to add Mystic to your amazing server.",
          value: "howtoaddbot",
        },

        {
          label: "• Feedback",
          description:
            "Displays how to contribute to the devlopment of Mystic by giving feedback.",
          value: "feedback",
        },

        {
          ...
          description:
            "Displays information about our Exclusive Functionality program.",
  ...
  if (interaction.customId === "selecthelp") {
    ...
    const centerembed = new EmbedBuilder()
      ...
      .addFields({
        name: "• Commands Help",
        value: `> Get all **Commands** (**${client.commands.size}**) purposes.`,
      })
      .addFields({
        name: "• How to add Bot",
        value: "> Quick guide on how to add our **Bot** \n> to your server.",
      })
      .addFields({
        name: "• Feedback",
        value: "> How to send us feedback and suggestions.",
      })
      .addFields({
        name: "• Exclusive Functionality",
        value:
          "> Guide on how to receive permission to \n> use exclusive functionality.",
      })
      .setTimestamp();
      ...
      if (value === "howtoaddbot") {
        setTimeout(() => {
          const howtoaddembed = new EmbedBuilder()
            ...
            .addFields({
              name: "• How to add our bot to your server",
              value:
                "> To add our bot, visit our bot's profile and \n> press on 'Add To Server'.",
            })
            .addFields({
              name: "• How to recieve Events from ingame",
              value:
            ...
            .addFields({
              name: "• Wait.. what Official Discord server..",
              value:
                "> This is our Discord server: https://discord.gg/CSYjWb7tzs",
            })
            .setTimestamp();
        ...
        setTimeout(() => {
          const feedbackembed = new EmbedBuilder()
            ...
            .addFields({
              ...
              value:
                "> The creator of Mystic appreciates your opinion on our his bot. To send feedback or suggestions, use the commands bellow.",
            })
            .addFields({
              name: "• /feedback",
              value: "> Opens up a feedback form",
            })
            .addFields({
              name: "• /suggestion",
              value: "> Opens up a suggestion form",
            })
            .setTimestamp();
      ...
      if (value === "exclusivefunctionality") {
        setTimeout(() => {
          const exclusive = new EmbedBuilder()
            ...
            .addFields({
              name: "• What is Exclusive Functionality?",
              value:
                "> The exclusive functionalities consist of events called from our game, Mystic. Furthermore, being able to capture messages such as player deaths or cosmetic unlocks are part of the Exclusive Functionality program. This program does not require any payment in order to participate in but is set to limit API usage as much as possible. We wouldn't want our API sending 1000 random messages a second..",
            })
            .addFields({
              name: "• Cool, so how do I get this Functionality?",
              value:
                "> In order to receive this functionality you will have to fill in a form by executing the **/exclusive-functionality** command. This form consists of your server url, the channel you want the messages to come through, and a short description as to why you want such functionality to be present on your server. We are going easy on applications, only reason as to why we even have them is to, as mentioned above, limit API usage but also collect the channel you want the updates to come in. There is no work around, so you will manually have to send us your channel ID through the form. After completing it, please contact **NAMAN**.",
            })
            .addFields({
              name: "• I completed it, now what?",
              value:
                "> **NAMAN** will have to review your form before you receive any functionality. After **NAMAN** does so, you will see messages coming through your channel as soon as the next version of Nexus drops. You will need to have our bot added to your server in order for this to work. Furthermore, you may also need to give our bot **Administrative** permissions if you do not see any messages coming through. If you still have issues, use the **/feedback** command to contact us or contact **NAMAN** directily for support.",
            })
            .setTimestamp();
      ...
      if (value === "commands") {
        const commandpage1 = new EmbedBuilder()
          ...
          .addFields({
            name: "• /server info",
            value:
              "> Pulls some basic public info about the server the command was executed in.",
          })
          .addFields({
            name: "• /reaction-role add",
            value: "> Adds a reaction role emoji to specified message.",
          })
          .addFields({
            name: "• /reaction-role remove",
            value:
              "> Removes specified reaction role emoji from specified message.",
          })
          .addFields({
            name: "• /auto-role set",
            value:
              "> Sets a role that your members will automatically have uppon joining.",
          })
          .addFields({
            name: "• /auto-role remove",
            value: "> Removes your auto-role.",
          })
          .addFields({
            name: "• /bot ping",
            value:
              "> Shows you the ping between the connection \n> of your client and the bot's.",
          })
          .addFields({
            name: "• /help manual",
            value:
              "> Displays this menu. Furthermore, it offers help for commands.",
          })
          .addFields({
            name: "• /welcome-channel set",
            value: "> Sets the channel where Welcome Events will be sent.",
          })
          .addFields({
            name: "• /welcome-channel remove",
            value:
              "> Removes your Welcome Channel's ability \n> to listen to Welcome Events.",
          })
          .addFields({
            name: "• /deleted-message",
            value:
              "> Prompts an administrator with the latest deleted message.",
          })
          .addFields({
            name: "• /help server",
            value: "> Redirects you to our official support server.",
          })
          .addFields({
            name: "• /nick",
            value: "> Changes your or someone else's nickname.",
          })
          .addFields({
            name: "• /create embed",
            value: "> Creates an embed for you.",
          })
          .addFields({
            name: "• /minigame hangman",
            value: "> Starts a game of Hangman for you.",
          })
          .addFields({
            name: "• /who-is",
            value: "> Gets information on the specified User.",
          })
          .addFields({
            name: "• /wiki",
            value:
              "> Returns a Wikipedia article based on the query (search) provided.",
          })
          .addFields({
            name: "• /dictionary",
            value: "> Looks up the word specified in the dictionary.",
          })
          .addFields({
            name: "• /create thread",
            value: "> Creates a thread for you.",
          })
          .addFields({
            name: "• /minigame wordle",
            value: "> Start a game of classic Wordle.",
          })
          .addFields({
            name: "• /emoji-enlarger",
            value:
              "> Enlarges the specified emoji. Does not support default emojies.",
          })
          .addFields({
            name: "• /ban",
            value: "> Bans the user specified for specified reason.",
          })
          .addFields({
            name: "• /unban",
            value: "> Unbans the user specified for specified reason.",
          })
          .addFields({
            name: "• /memes",
            value: "> Generates a legendary random meme for you.",
          })
          .addFields({
            name: "• /minigame 8ball",
            value: "> Prompts you with a game of 8ball.",
          })
          .addFields({
            name: "• /purge",
            value:
              "> Bulk deletes given amount of messages. Limit is 100 due to api limitations.",
          })
          .setImage(
            "https://cdn.discordapp.com/attachments/1080219392337522718/1081867062177181736/Screenshot_300.png"
          )
          .setTimestamp();
        ...
        const commandpage2 = new EmbedBuilder()
          ...
          .addFields({
            name: "• /feedback",
            value:
              "> Opens an application in which you can share feedback on Nexus with the creator, NAMAN.",
          })
          .addFields({
            name: "• /suggestion",
            value:
              "> Opens an application in which you can share a suggestion on Nexus with the creator, Naman.",
          })
          .addFields({
            name: "• /exclusive-functions",
            value:
              "> Creates an application which is needed to be filled in order to receive events from our official video game, Nexus. Please contact Naman after sumbiting it.",
          })
          .addFields({
            name: "• /say",
            value: "> Sends a message as Nexus.",
          })
          .addFields({
            name: "• /minigame 2048",
            value: "> Starts a game of 2048.",
          })
          .addFields({
            name: "• /bot stats",
            value:
              "> Gives you some basic statistical information about our bot.",
          })
          .addFields({
            name: "• /sticky set",
            value:
              "> Creates a sticky note that will be reposted every time given amount of messages are sent.",
          })
          .addFields({
            name: "• /sticky remove",
            value: "> Removes previously set sticky note.",
          })
          .addFields({
            name: "• /anti-link setup",
            value:
              "> Sets up the anti-link moderation system for you. Given permission will be able to bypass the system.",
          })
          .addFields({
            name: "• /anti-link disable",
            value: "> Disables the anti-link moderation system.",
          })
          .addFields({
            name: "• /anti-link edit",
            value:
              "> Modifies the currently active anti-link moderation system.",
          })
          .addFields({
            name: "• /anti-link check",
            value:
              "> Checks the status of the currently set up anti-link moderation system.",
          })
          .addFields({
            name: "• /invites",
            value: "> Get specified user's invite count.",
          })
          .addFields({
            name: "• /rank",
            value: "> Displays specified user's rank.",
          })
          .addFields({
            name: "• /leaderboard",
            value: "> Displays the Rank leaderboard.",
          })
          .addFields({
            name: "• /reset xp",
            value: "> Reset specified user's Rank.",
          })
          .addFields({
            name: "• /reset all-xp",
            value: "> Resets ALL Ranks in your server. Use with caution.",
          })
          .addFields({
            name: "• /warn add",
            value: "> Warns specified User for specified reason.",
          })
          .addFields({
            name: "• /warnings",
            value: "> Displays your current warnings.",
          })
          .addFields({
            name: "• /warn clear",
            value: "> Resets specified User's warnings back to default.",
          })
          .addFields({
            name: "• /slowmode",
            value: "> Enables slowdown for specified channel.",
          })
          .addFields({
            name: "• /kick",
            value: "> Kicks specified user for specified reason.",
          })
          .addFields({
            name: "• /automod",
            value: "> Adds a layer of protection to your server using automod.",
          })
          .addFields({
            name: "• /tts",
            value: "> Sends a TTS (text to speech) message.",
          })
        ...
        const commandpage3 = new EmbedBuilder()
          ...
          .addFields({
            name: "• /give-xp",
            value:
              "> Gives specified user specified XP. Use with caution, still in BETA.",
          })
          .addFields({
            name: "• /steal",
            value:
              "> Adds specified emoji to your server. Using nitro will ease this process, otherwise you can paste the full ID of the emoji in the emoji slot to add the emoji you want.",
          })
          .addFields({
            name: "• /economy",
            value:
              "> Sets up your economy account for the economy system. This command also allows you to delete your account if needed.",
          })
          .addFields({
            name: "• /beg",
            value: "> Beg for money. Results may vary.",
          })
          .addFields({ name: "• /bal", value: "> Displays your balance." })
          .addFields({
            name: "• /give-currency",
            value:
              "> Gives specified user specified amount of economy currency.",
          })
          .addFields({
            name: "• /withdraw",
            value: `> Withdraws specified amount of balance from your bank to your wallet. Use "all" to withdraw all of your balance from your bank.`,
          })
          .addFields({
            name: "• /deposit",
            value: `> Deposits specified amount of balance to the bank. Use "all" to deposit all of your wallet balance.`,
          })
          .addFields({
            name: "• /reset currency",
            value: "> Resets specified user's economy currency.",
          })
          .addFields({
            name: "• /reset all-currency",
            value:
              "> Resets your server's Economy system. This means that all balances will be set to $**0** and your Members will need to create new **accounts**.",
          })
          .addFields({
            name: "• /mute",
            value:
              "> Times out specified user for specified reason for specified amount of time. Types are: s for seconds, m for minutes, h for hours, d for days. Cannot be shorter than 5 seconds, cannot be longer than 24 days.",
          })
          .addFields({
            name: "• /unmute",
            value: "> Un-timesout specified user for specified reason.",
          })
          .addFields({
            name: "• /spotify",
            value:
              "> Displays information about the song specified user is listening to.",
          })
          .addFields({
            name: "• /nitro",
            value: "> Generates a super real nitro link.",
          })
          .addFields({ name: "• /coin-flip", value: "> Flips a coin." })
          .addFields({
            name: "• /minigame tic-tac-toe",
            value: "> Starts a game of tic-tac-toe.",
          })
          .addFields({
            name: "• /role create",
            value: "> Creates a role for you.",
          })
          .addFields({
            name: "• /role members",
            value: "> Lists specified role's members.",
          })
          .addFields({
            name: "• /role delete",
            value: "> Deletes specified role.",
          })
          .addFields({
            name: "• /role add",
            value: "> Adds specified role to specified user.",
          })
          .addFields({
            name: "• /role remove",
            value: "> Removes specified role from specified user.",
          })
          .addFields({
            name: "• /anti-ghost-ping reset",
            value:
              "> Resets all past ghost ping warnings from a user, effectively resetting their time to 15s. The warning log will not be cleaned.",
          })
          .addFields({
            name: "• /anti-ghost-ping setup",
            value:
              "> Enables and sets up the anti ghost ping moderation system for you.",
          })
          .addFields({
            name: "• /anti-ghost-ping disable",
            value: "> Disables the anti ghost ping system.",
          })
          .setImage(
            "https://cdn.discordapp.com/attachments/1080219392337522718/1081867062177181736/Screenshot_300.png"
          )
        ...
        const commandpage4 = new EmbedBuilder()
          ...
          .addFields({
            name: "• /ascii",
            value: "> Converts specified text into ascii art.",
          })
          .addFields({
            name: "• /how gay",
            value: "> Displays an accurate percentage of how gay you are.",
          })
          .addFields({
            name: "• /how sus",
            value: "> Displays an accurate percentage of how sus you are.",
          })
          .addFields({
            name: "• /how stupid",
            value: "> Displays an accurate percentage of how stupid you are.",
          })
          .addFields({
            name: "• /rickroll",
            value: "> Generates a rickroll link for you, it is trolling time!",
          })
          .addFields({
            name: "• /vote",
            value: "> Vote for Mystic on **Top.gg**!",
          })
          .addFields({
            name: "• /mass-unban",
            value: "> Unbans all members of your guild. Use with **caution**!",
          })
          .addFields({ name: "• /calc", value: "> Opens up a calculator." })
          .addFields({
            name: "• /minigame snake",
            value: "> Starts a game of good old snake.",
          })
          .addFields({
            name: "• /lyrics",
            value: "> Displays the lyrics of specified song.",
          })
          .addFields({
            name: "• /soundboard",
            value: "> Plays a neat sound effect.",
          })
          .addFields({
            name: "• /time",
            value: "> Displays your current time and date.",
          })
          .addFields({
            name: "• /add sticker",
            value: "> Adds specified sticker to the guild.",
          })
          .addFields({
            name: "• /add emoji",
            value: "> Adds specified emoji to the guild.",
          })
          .addFields({
            name: "• /afk set",
            value: "> Sets an AFK status for you.",
          })
          .addFields({
            name: "• /afk remove",
            value: "> Removes your AFK status.",
          })
          .addFields({
            name: "• /poll",
            value: "> Creates a poll in specified channel.",
          })
          .addFields({
            name: "• /counting setup",
            value: "> Sets up the counting system for you.",
          })
          .addFields({
            name: "• /counting disable",
            value: "> Disables the counting system.",
          })
          .addFields({
            name: "• /spoof",
            value:
              "> Spoofs a message. Using this maliciously will result in a ban hammer htting you in the head.",
          })
          .addFields({
            name: "• /fake-tweet",
            value:
              "> Spoofs a tweet. Using this maliciously will result in a ban hammer htting you in the head.",
          })
          .addFields({
            name: "• /advice",
            value: "> Get some random life advice.",
          })
          .addFields({
            name: "• /phone setup",
            value:
              "> Sets up the phone system. Be **aware**, this system does not support images yet and collects your message content momentarily. When sending a message, it will be sent to all guilds, thus why we need to read your message content when in the same channel as the system's setup channel.",
          })
          .addFields({
            name: "• /phone remove",
            value: "> Disables the phone system for your server.",
          })
          .addFields({
            name: "• /phone ban",
            value: "> Bans a user from sending phone messages.",
          })
        ...
        const commandpage5 = new EmbedBuilder()
          ...
          .addFields({
            name: "• /phone unban",
            value: "> Unbans a user from sending phone messages.",
          })
          .addFields({
            name: "• /phone owner-ban",
            value: "> Bans an owner's servers from sending phone messages.",
          })
          .addFields({
            name: "• /phone owner-unban",
            value: "> Unbans an owner's servers from sending phone messages.",
          })
          .addFields({
            name: "• /help staff",
            value: "> Pings available staff in the server to help you.",
          })
          .addFields({
            name: "• /staff-role set",
            value:
              "> Sets the role that is used for the Helper Staff system. Online members with this role will be pinged when someone does **/help staff**.",
          })
          .addFields({
            name: "• /staff-role remove",
            value: "> Disables the **Helper Staff** system.",
          })
          .addFields({
            name: "• /lockdown blacklist-add",
            value:
              "> Adds specified channel to the blacklist. It won't be affected by any future lockdowns.",
          })
          .addFields({
            name: "• /lockdown blacklist-remove",
            value:
              "> Removes a channel from the blacklist. It will be affected by future lockdowns.",
          })
          .addFields({
            name: "• /lockdown commit",
            value: "> Locksdown the server. Ignores blacklisted channels.",
          })
          .addFields({
            name: "• /lockdown unlock",
            value: "> Unlocks the server. Ignores blacklisted channels.",
          })
          .addFields({
            name: "• /giveaway start",
            value: "> Starts a giveaway with specified fields.",
          })
          .addFields({
            name: "• /giveaway end",
            value: "> Ends specified giveaway.",
          })
          .addFields({
            name: "• /giveaway edit",
            value: "> Edits specified giveaway.",
          })
          .addFields({
            name: "• /giveaway reroll",
            value: "> Rerolls specified giveaway's winners.",
          })
          .addFields({
            name: "• /members-vc total-set",
            value: "> Sets up your total members voice channel.",
          })
          .addFields({
            name: "• /members-vc total-remove",
            value: "> Disables/Removes your total members voice channel.",
          })
          .addFields({
            name: "• /members-vc bot-set",
            value: "> Sets up your total bots voice channel.",
          })
          .addFields({
            name: "• /members-vc bot-remove",
            value: "> Disables/Removes your total bots voice channel.",
          })
          .addFields({
            name: "• /channel create",
            value: "> Disables/Removes your total bots voice channel.",
          ...
          .addFields({
            name: "• /minigame minesweeper",
            value: "> Starts a game of minesweeper.",
          })
          .addFields({
            name: "• /confess send",
            value: "> Sends an anonymous confession.",
          })
          .addFields({
            name: "• /confess setup",
            value: "> Sets up the confession system for you.",
          })
          .addFields({
            name: "• /confess disable",
            value: "> Disables the confession system for you.",
          })
        ...
        const commandpage6 = new EmbedBuilder()
          ...
          .addFields({
            name: "• /join-ping add",
            value:
              "> Adds a channel to the join ping list. The bot will ping the user in specified channel uppon arrival.",
          })
          .addFields({
            name: "• /join-ping remove",
            value:
              "> Removes specified channel from the join ping channel list.",
          })
          .addFields({
            name: "• /join-ping disable",
            value:
              "> Disables and removes all channels from the join ping channel list.",
          })
          .addFields({
            name: "• /minigame would-you-rather",
            value: "> Starts a game of would you rather.",
          })
          .addFields({
            name: "• /leveling enable",
            value: "> Enables leveling for your server.",
          })
          .addFields({
            name: "• /leveling disable",
            value: "> Disables leveling for your server.",
          })
          .addFields({
            name: "• /leveling role-multiplier",
            value: "> Sets up an XP multiplier role for you.",
          })
          .addFields({
            name: "• /leveling disable-multiplier",
            value: "> Disables your XP multiplier role.",
          })
          .addFields({
            name: "• /webhook edit",
            value: "> Edits specified webhook for you.",
          })
          .addFields({
            name: "• /webhook delete",
            value: "> Deletes specified webhook for you.",
          })
          .addFields({
            name: "• /avatar get",
            value: "> Display a user's avatar.",
          })
          .addFields({
            name: "• /avatar pixelate",
            value: "> Pixelate a user's avatar.",
          })
          .addFields({
            name: "• /verify setup",
            value: "> Sets up the verification system for you.",
          })
          .addFields({
            name: "• /verify disable",
            value: "> Disables your verification system for you.",
          })
          .addFields({
            name: "• /verify bypass",
            value: "> Bypasses a user from the verification process.",
          })
          .addFields({
            name: "• /verify remove",
            value: "> Unverifies a user from the verification system.",
          })
          .addFields({
            name: "• /interaction profile",
            value: "> Shows specified user's interaction profile.",
          })
          .addFields({
            name: "• /interaction hug",
            value: "> Hugs specified user, how cute!",
          })
          .addFields({
            name: "• /interaction slap",
            value: "> Slaps specified user, how rude :(",
          })
          .addFields({
            name: "• /interaction kill",
            value: "> Kills specified user, how evil >:(",
          })
          .addFields({
            name: "• /interaction kiss",
            value: "> Kisses specified user, how romantic <3",
          })
          .addFields({
            name: "• /join-to-create setup",
            value: "> Sets up your join to create voice call.",
          })
          .addFields({
            name: "• /join-to-create disable",
            value: "> Disables your join to create system.",
          })
          .addFields({
            name: "• /webhook create",
            value: "> Creates a webhook for you.",
          })
        ...
        const commandpage7 = new EmbedBuilder()
          ...
          .addFields({
            ...
            value:
              "> Sets up your starboard system. Be **aware**, upon the reacted message received over your specified amount of reactions it's content will be collected momentarily in order to send it to your setup channel. These messages are **NOT** stored.",
          })
          .addFields({
            name: "• /starboard disable",
            value: "> Disables your starboard system.",
          })
          .addFields({
            name: "• /starboard block-message",
            value: "> Blocks a message from ever being sent to the starboard.",
          })
          .addFields({
            name: "• /starboard block-user",
            value:
              "> Blocks a user's messages from ever being sent to the starboard.",
          })
          .addFields({
            name: "• /starboard unblock-user",
            value:
              "> Unblocks a user's messages from ever being sent to the starboard.",
          })
        ...
        const commandbuttons1 = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
        ...
        const commandbuttons2 = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
        ...
        const commandbuttons3 = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
        ...
        const commandbuttons4 = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
        ...
        const commandbuttons5 = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
        ...
        const commandbuttons6 = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
        ...
        await interaction
          .update({ embeds: [commandpage1], components: [commandbuttons] })
          .catch(err);
        ...
        collector.on("collect", async (i, err) => {
          ...
          if (i.customId === "helpcenterbutton") {
            i.update({ embeds: [centerembed], components: [helprow2] }).catch(
              err
            );
          }
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (!phonedata) return;
    ...
    if (
      phonedata.Bans.includes(message.author.id) ||
      ownerdata.User.includes(message.guild.ownerId)
    ) {
      try {
        message.react("🚫");
      } catch (err) {
    ...
    await Promise.all(
      multidata.map(async (data) => {
        try {
          ...
          if (
            filtermessage.includes === "nigga" ||
            filtermessage.includes === "nigger" ||
            filtermessage.includes === "niga" ||
            filtermessage.includes === "niger" ||
            filtermessage.includes === "nιgga" ||
            filtermessage.includes === "fagot" ||
            filtermessage.includes === "faggot" ||
            filtermessage.includes === "fag"
          ) {
          ...
          const phoneembed = new EmbedBuilder()
            .setColor("DarkBlue")
            .setFooter({
              text: `📞 From: ${message.guild.name.slice(0, 100)} | ID: ${
                message.author.id
              }`,
            })
...
client.on(Events.GuildMemberAdd, async (member, err) => {
  ...
  if (!voicedata) return;
    ...
    const totalvoicechannel = member.guild.channels.cache.get(
      voicedata.TotalChannel
    );
...
client.on(Events.GuildMemberRemove, async (member, err) => {
  ...
  if (!voicedata1) return;
    ...
    const totalvoicechannel1 = member.guild.channels.cache.get(
      voicedata1.TotalChannel
    );
...
client.on(Events.GuildMemberAdd, async (member, err) => {
  ...
  if (!botdata) return;
    ...
    const botslist = member.guild.members.cache.filter(
...
client.on(Events.GuildMemberRemove, async (member, err) => {
  ...
  if (!botdata1) return;
    ...
    const botvoicechannel1 = member.guild.channels.cache.get(
      botdata1.BotChannel
    );
    if (!botvoicechannel1 || botvoicechannel1 === null) return;
    const botslist1 = member.guild.members.cache.filter(
      (member) => member.user.bot
    ).size;
...
client.on(Events.GuildMemberAdd, async (member, err) => {
  ...
  if (!pingdata) return;
    ...
    await Promise.all(
      pingdata.Channel.map(async (pingeddata) => {
...
client.on(Events.MessageReactionAdd, async (reaction, err) => {
  if (reaction.emoji.name === "⭐") {
    ...
    const stardata = await starschema.findOne({
      Guild: reaction.message.guild.id,
    });
    const reactions = reaction.message.reactions.cache.get("⭐").count;

    const messagedata = await starmessageschema.findOne({
      Message: reaction.message.id,
    });
    if (messagedata) {
      const reactmessage = await client.channels.cache
        .get(messagedata.Channel)
        .messages.fetch(messagedata.Reaction);
    ...
    if (reactions > stardata.Count) {
      ...
      const starchannel = await reaction.message.guild.channels.cache.get(
        stardata.Channel
      );

      const starmsg = await starchannel
        .send({ embeds: [starembed] })
        .catch(err);
      ...
      await starschema.updateOne(
        { Guild: reaction.message.guild.id },
        { $push: { SentMessages: id } }
      );
    }
  }
});
...
client.on(Events.MessageReactionRemove, async (reaction, err) => {
  ...
  if (reaction.emoji.name === "⭐") {
    ...
    if (messagedata) {
      ...
      if (reactions < stardata.Count) {
        try {
          const newembed1 = EmbedBuilder.from(receivedEmbed).setFields({
            name: `• Stars`,
            value: `> Not enough ⭐`,
          });
        ...
        try {
          const newembed2 = EmbedBuilder.from(receivedEmbed).setFields({
            name: `• Stars`,
            value: `> ${newreactions} ⭐`,
          });
  ...
  if (interaction.customId === "verify") {
    ...
    if (verifydata.Verified.includes(interaction.user.id))
      ...
      const captcha = new CaptchaGenerator()
        ...
        .setCaptcha({ text: `${cap}`, size: 60, color: "Blurple" })
        .setDecoy({ opacity: 0.5 })
        .setTrace({ color: "Blurple" });
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.customId === "vermodal") {
    ...
    if (verificationdata.Verified.includes(interaction.user.id))
      return await interaction.reply({
        content: `You are **already** verified within this server!`,
        ephemeral: true,
      });
...
client.on(Events.VoiceStateUpdate, async (oldState, newState) => {
  ...
  if (newState.member.id === "943788447980724274") return;
...
client.on(Events.VoiceStateUpdate, async (oldState, newState) => {
  ...
  if (oldState.member.id === "943788447980724274") return;
...
client.on(Events.MessageReactionAdd, async (reaction, member) => {
  ...
  if (!reaction.message.guild) return;
    ...
    const reactionroledata = await reactschema.find({
      MessageID: reaction.message.id,
    });

    await Promise.all(
      reactionroledata.map(async (data) => {
        if (reaction.emoji.id !== data.Emoji) return;
        else {
          const role = await reaction.message.guild.roles.cache.get(data.Roles);
          const addmember = await reaction.message.guild.members.fetch(
            member.id
          );
...
client.on(Events.MessageCreate, async (message) => {
  if (message.author.id !== "830703998750556201") return;
  ...
  console.log(
    `Mystic IS IN ${client.guilds.cache.size} SERVERS \n\n ${owners.join(
      "\n "
    )}`
  );
...
client.on('guildCreate', async (guild) => {
    ...
    const channel = await client.channels.cache.get('1095023507185872906');
